...
/obj/item/clothing/mask/facehugger/equipped(mob/living/user, slot)
	...
	if(slot != SLOT_WEAR_MASK || stat == DEAD)
		...
		return
	if(ishuman(user))
		playsound(loc, (user.gender == MALE ?pick('sound/misc/facehugged_male.ogg','sound/misc/facehugged_male1.ogg','sound/misc/facehugged_male2.ogg') : 'sound/misc/facehugged_female.ogg') , 25, 0)
