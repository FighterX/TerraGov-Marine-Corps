...
/client/verb/looc(msg as text)
	...
	if(admin && isobserver(mob))
		message = "<font color='#6699CC'><span class='ooc'><span class='prefix'>LOOC:</span> [usr.client.holder.fakekey ? "Administrator" : usr.client.key]: <span class='message linkify emojify'>[msg]</span></span></font>"
		for(var/mob/M in range(mob))
			to_chat(M, message)
	else
		message = "<font color='#6699CC'><span class='ooc'><span class='prefix'>LOOC:</span> [mob.name]: <span class='message linkify emojify'>[msg]</span></span></font>"
		for(var/mob/M in range(mob))
			to_chat(M, message)
	...
	for(var/client/C in GLOB.admins)
		...
		if(C.prefs.toggles_chat & CHAT_LOOC)
			to_chat(C, "<font color='#6699CC'><span class='ooc'><span class='prefix'>LOOC: [ADMIN_TPMONTY(mob)]</span>: <span class='message linkify emojify'>[msg]</span></span></font>")
